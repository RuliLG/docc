...
class FileContentResponse(BaseModel):
    ...
    total_lines: int = Field(description="Total number of lines in the file", ge=0)
    from_line: Optional[int] = Field(
        None, description="Starting line number for filtered content (1-based)"
    )
    to_line: Optional[int] = Field(
        None, description="Ending line number for filtered content (1-based)"
    )
...
class RootResponse(BaseModel):
    ...
    providers: dict = Field(description="Available providers information")
...
class ServiceStatus(BaseModel):
    ...
    installed: Optional[bool] = Field(
        None, description="Whether the service is installed"
    )
    configured: Optional[bool] = Field(
        None, description="Whether the service is configured"
    )
    accessible: Optional[bool] = Field(
        None, description="Whether the service is accessible"
    )
    version: Optional[str] = Field(None, description="Service version if available")
    error: Optional[str] = Field(None, description="Error message if any")
    api_key_set: Optional[bool] = Field(
        None, description="Whether API key is configured"
    )
...
class SystemCheckResponse(BaseModel):
    ...
    requirements_met: RequirementsMet = Field(
        description="Status of minimum requirements"
    )
...
class QuickSystemCheckResponse(BaseModel):
    ...
    system_ready: bool = Field(
        description="Whether the system meets minimum requirements"
    )
