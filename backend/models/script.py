...
class TextBlock(BaseModel):
    ...
    markdown: str = Field(
        description="Markdown content to be narrated",
        min_length=1
    )
...
class CodeBlock(BaseModel):
    ...
    markdown: str = Field(
        description="Markdown explanation of the code block",
        min_length=1
    )
...
class ScriptRequest(BaseModel):
    ...
    question: str = Field(
        description="Question about the repository to answer",
        min_length=1
    )
    ai_provider: Optional[str] = Field(
        None,
        description="AI provider to use for code analysis (claude_code, opencode)"
    )
...
class ScriptResponse(BaseModel):
    ...
    audio_files: Optional[List[str]] = Field(
        None,
        description="URLs to generated audio files for each script block"
    )
