...
class TestClaudeProvider:
    ...
    @pytest.mark.asyncio
    @patch('subprocess.run')
    async def test_analyze_repository_success_with_json(self, mock_run, claude_provider):
        ...
        mock_run.assert_called_once_with(
            ["claude", "-p", "Test prompt", "--no-stream"],
            capture_output=True,
            text=True,
            check=True,
            cwd="/test/repo"
    ...
    @pytest.mark.asyncio
    @patch('subprocess.run')
    async def test_analyze_repository_success_without_json(self, mock_run, claude_provider):
        """Test when response doesn't contain valid JSON."""
        mock_run.return_value = Mock(
            returncode=0,
            stdout="This is a plain text response without JSON",
            stderr=""
        )
    ...
    @pytest.mark.asyncio
    @patch('subprocess.run')
    async def test_analyze_repository_command_failed(self, mock_run, claude_provider):
        """Test when Claude CLI command fails."""
        mock_run.side_effect = subprocess.CalledProcessError(
            returncode=1,
            cmd=["claude"],
            stderr="Command failed: invalid prompt"
        )
        
        with patch.object(claude_provider, 'is_available', return_value=True):
            with pytest.raises(RuntimeError, match="Claude Code CLI failed with exit code 1: Command failed: invalid prompt"):
                await claude_provider.analyze_repository(
                    "/test/repo",
                    "Test question?",
                    "Test prompt"
    ...
    async def test_analyze_repository_unexpected_error(self, mock_run, claude_provider):
        ...
        with patch.object(claude_provider, 'is_available', return_value=True):
            with pytest.raises(RuntimeError, match="Error running Claude Code CLI: Unexpected error"):
                await claude_provider.analyze_repository(
                    "/test/repo",
                    "Test question?",
                    "Test prompt"
